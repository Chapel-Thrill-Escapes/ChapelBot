...
if (http_status(bookingResponse)$category == "Success" & http_status(customerResponse)$category == "Success") {
  ...
  for (booking in 1:nrow(bookingdata)) {
    ...
    for (i in 1:nrow(personinfo)){
      if (personinfo[[2]][[i]] == "MPJWRE") {
        ...
        print(paste("Match found for PID:", PID))
        }
      }
      else {
        next
  ...
  if(length(unique(notoncampus$BookingID)) > 0) {
    for (i in 1:length(unique(notoncampus$BookingID))){
      ...
      reason <- paste0('ChapelBot has cancelled your booking due to no on-campus UNC student match being found for PID(s): ', PIDs, '. You can rebook at chapelthrillescapes.com/book with a PID matching the names of the on-campus student(s) exactly as they appear on their Student ID or for off-campus students select "Student ($16)." If you think this cancellation is incorrect or have any questions, please send our team an email at admin@chapelthrillescapes.com.')
      response <- DELETE(
